...
{if $cart->purchases}
    ...
    <form id="captcha_id" method="post" name="cart" class="fn_validate_cart">
        ...
        <div class="block padding">
            ...
            {* Submit button *}
            <input class="button g-recaptcha" type="submit" name="checkout" data-language="cart_checkout" onclick="openOrder() {if $settings->captcha_type == "invisible"}data-sitekey="{$settings->public_recaptcha_invisible}" data-badge='bottomleft' data-callback="onSubmit"{/if} value="{$lang->cart_checkout}">
        </div>
    </form>
